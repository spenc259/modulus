<?php
include_once get_template_directory() . '/inc/scss/scss.inc.php'; // Sass Compiler (vendor)
   ...
   */
   ...
   /**
    ...
    public function compile() {
        global $scssc, $cache;
        $cache = get_template_directory() . '/assets/scss/cache/';
        ...
        function compiler($in, $out, $instance) {
            ...
            if (is_writable($cache)) {
                try {
                    ...
                    $result = file_put_contents($cache.basename($out), $css);
                    // print_r($result);
                } catch (Exception $e) {
                    $errors = array (
                    'file' => basename($in),
                ...
                $errors = array (
                'file' => $cache,
                'message' => "File Permission Error, permission denied. Please make the cache directory writable." . $cache
                );
                array_push($instance->compile_errors, $errors);
            }
        }
